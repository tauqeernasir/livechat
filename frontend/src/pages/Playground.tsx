...
export default function Playground() {
  ...
  return (
    <Layout fullWidth>
      <div className="flex h-screen bg-gray-50 dark:bg-slate-950 overflow-hidden border-t border-gray-200 dark:border-slate-900">
        {/* Sessions Sidebar */}
        <div className="w-80 border-r border-gray-200 dark:border-slate-900 bg-gray-50 dark:bg-slate-900/20 flex flex-col min-w-[250px] w-[250px]">
          ...
          <div className="flex-grow overflow-y-auto p-3 space-y-2 custom-scrollbar">
            {isLoadingSessions ? (
              <div className="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-slate-500 gap-3">
                <Loader2 className="w-6 h-6 animate-spin" />
                <span className="text-xs">Loading history...</span>
              </div>
            ) : sessions.length === 0 ? (
              <div className="text-center py-10 text-gray-300 dark:text-slate-600 px-4">
              ...
              sessions.map(session => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <MessageSquare className={`w-4 h-4 flex-shrink-0 ${activeSessionId === session.id ? 'text-indigo-400' : 'text-gray-300 dark:text-slate-600'}`} />
        ...
        {/* Main Chat Area */}
        <div className="flex-grow flex flex-col bg-gray-50 dark:bg-slate-950">
          {/* Header / Agent Selector */}
          <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-900 flex items-center justify-between bg-gray-50 dark:bg-slate-900/10">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl relative group">
                <Bot className="w-4 h-4 text-indigo-500" />
                <select
                  value={selectedWorkspaceId || ''}
                  onChange={(e) => setSelectedWorkspaceId(Number(e.target.value))}
                  className="bg-transparent text-sm font-semibold text-gray-900 dark:text-white outline-none cursor-pointer pr-6 appearance-none"
                >
                  {workspaces.map(ws => (
                    <option key={ws.id} value={ws.id} className="bg-white dark:bg-slate-900">{ws.name}</option>
                  ))}
                </select>
                <ChevronDown className="w-3.5 h-3.5 text-gray-400 dark:text-slate-500 absolute right-3 pointer-events-none" />
              </div>
              <span className="text-[10px] font-bold text-gray-300 dark:text-slate-600 uppercase tracking-widest">Live Test Mode</span>
            </div>
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-8 custom-scrollbar">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto">
                ...
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Ready to test?</h3>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          ...
                          */}
                          {extractSources(msg.content).map((source, sIdx) => (
                            <div
                              key={sIdx}
                              className="flex items-center gap-1.5 px-2.5 py-1 bg-white dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg text-[10px] font-bold text-gray-400 dark:text-slate-500 hover:text-indigo-400 hover:border-indigo-500/30 transition-all cursor-default"
                            >
          ...
          <div className="p-6 bg-gray-50 dark:bg-slate-950">
            <div className="max-w-4xl mx-auto relative group">
              ...
              <div className="relative flex items-center bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-2 pl-4 transition-all focus-within:border-indigo-500/50 focus-within:ring-4 focus-within:ring-indigo-500/5 shadow-xl">
                <input
                  type="text"
                  value={input}
                  ...
                  className="flex-grow bg-transparent border-none outline-none text-gray-900 dark:text-white text-sm py-2 disabled:opacity-50"
                />
                <button
                  onClick={handleSend}
