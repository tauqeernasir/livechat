        ...
        <Layout>
            <div className="flex flex-col gap-8">
                ...
                <header className="flex items-end justify-between">
                    <div>
                        ...
                        <p className="text-slate-400">Connect external APIs so your agent can fetch live data.</p>
                    </div>
                    <button
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-900/50 border border-slate-800 rounded-2xl py-3 pl-12 pr-4 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 transition-all"
                    />
                </div>
                ...
                <div className="space-y-4">
                    {isLoading ? (
                        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-20 flex flex-col items-center justify-center text-slate-500">
                        ...
                        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-20 flex flex-col items-center justify-center text-center">
            ...
            {isCreateOpen && (
                <div className="fixed inset-0 z-50 overflow-hidden">
                    <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm" onClick={() => { setIsCreateOpen(false); resetCreateForm(); }} />
                        ...
                        <div className="w-screen max-w-2xl">
                            <div className="h-full flex flex-col bg-slate-900 border-l border-slate-800 shadow-2xl">
                                <header className="p-6 border-b border-slate-800 flex items-center justify-between">
                                    <div>
                                        <h2 className="text-xl font-bold text-white">Connect API</h2>
                                        <p className="text-sm text-slate-500">Provide an OpenAPI spec to register callable endpoints.</p>
                                    </div>
                                    <button onClick={() => { setIsCreateOpen(false); resetCreateForm(); }} className="p-2 text-slate-400 hover:text-white transition-colors">
                                ...
                                <div className="flex-grow overflow-y-auto p-6 space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-semibold text-slate-300">Integration Name</label>
                                        <input
                                            type="text"
                                            placeholder="e.g. Order Service"
                                            value={createName}
                                            onChange={(e) => setCreateName(e.target.value)}
                                            className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 px-4 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 transition-all"
                                        />
                                    </div>

                                    {/* Spec Source */}
                                    <div className="space-y-2">
                                        <label className="text-sm font-semibold text-slate-300">OpenAPI Spec</label>
                                        ...
                                        {createSpecMode === 'url' ? (
                                            ...
                                            <textarea
                                                ...
                                                rows={10}
                                                className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 px-4 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 transition-all font-mono text-sm resize-none"
                                            />
                                        )}
                                    </div>
                                    ...
                                    {/* Auth */}
                                    <div className="space-y-2">
                                        <label className="text-sm font-semibold text-slate-300">Authentication</label>
                                        <select
                                            value={createAuthType}
                                            onChange={(e) => setCreateAuthType(e.target.value)}
                                            className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 px-4 text-white focus:outline-none focus:ring-2 focus:ring-indigo-600/20 transition-all"
                                        >
                                    ...
                                    {createAuthType !== 'none' && (
                                        <>
                                            {createAuthType === 'header' && (
                                                <div className="space-y-2">
                                                    <label className="text-sm font-semibold text-slate-300">Header Name</label>
                                                    <input
                                                        type="text"
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-semibold text-slate-300">
                                            Base URL <span className="text-slate-500 font-normal">(optional — auto-detected from spec)</span>
                                        </label>
                                        <input
                                            type="url"
                                ...
                                <footer className="p-6 border-t border-slate-800 bg-slate-900/50 flex gap-3">
                                    <button
                                        ...
                                        className="flex-grow py-3 rounded-xl font-bold text-slate-400 hover:text-white hover:bg-slate-800 transition-all"
                                    >
                                        Cancel
                                    </button>
                                    <button
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-sm">
            ...
            <div className="px-6 py-5 flex items-center justify-between">
                <div className="flex items-center gap-4 flex-grow min-w-0">
                    <div className="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center border border-slate-700/50 flex-shrink-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                            {integration.base_url && (
                                <span className="text-[11px] text-slate-500 truncate max-w-[300px]">{integration.base_url}</span>
                            )}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button
                        ...
                        className={`p-2 transition-colors ${integration.enabled ? 'text-emerald-400 hover:text-emerald-300' : 'text-slate-600 hover:text-slate-400'}`}
                    >
            ...
            {/* Edit Panel */}
            {isEditing && (
                <div className="px-6 pb-5 border-t border-slate-800/50 pt-5 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-600/20"
                            />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-slate-400">Auth Type</label>
                            <select
                                value={editAuthType}
                        ...
                        {editAuthType !== 'none' && (
                            <>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-semibold text-slate-400">New Credential</label>
                                    <input
                                        type="password"
                                        placeholder="Leave blank to keep current"
                                        value={editCredentials}
                                        onChange={(e) => setEditCredentials(e.target.value)}
                                        className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2 px-3 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-600/20"
                                    />
                                </div>
                            </>
                        )}
            ...
            {isExpanded && integration.operations.length > 0 && (
                <div className="border-t border-slate-800/50">
                    <div className="px-6 py-3 bg-slate-800/20 flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Endpoints</span>
                        <div className="flex items-center gap-2">
                            <button
                    ...
                    <div className="divide-y divide-slate-800/50">
                        {integration.operations.map((op) => (
                            <div key={op.id} className="px-6 py-3 flex items-center justify-between hover:bg-slate-800/20 transition-colors">
                                <div className="flex items-center gap-3 min-w-0">
                                    <MethodBadge method={op.method} />
                                    <span className="text-sm text-slate-300 font-mono truncate">{op.path}</span>
                                    {op.summary && (
                                        <span className="text-xs text-slate-500 truncate max-w-[250px]">{op.summary}</span>
                                    )}
                                </div>
                                <button
...
function StatusBadge({ status }: { status: Integration['status'] }) {
    const configs = {
        pending: { icon: <Clock className="w-3 h-3" />, color: 'bg-slate-800 text-slate-400 border-slate-700' },
        ...
        disabled: { icon: <PowerOff className="w-3 h-3" />, color: 'bg-slate-800 text-slate-500 border-slate-700' },
    };
...
function MethodBadge({ method }: { method: string }) {
    const colors: Record<string, string> = {
        ...
        HEAD: 'bg-slate-700/50 text-slate-400 border-slate-600',
